...
## 검증 완료 (2026-07-16)
- [x] ~~대화 로그 조회 UI~~ — 운영자가 봇별 세션 목록(좌) + 트랜스크립트(우)를 열람. 인증·테넌트 격리 `GET /api/workspaces/{ws}/bots/{bot}/sessions`(+`/{sid}/messages`), 대시보드 카드 "대화 로그 보기" 진입점 → `/app/bots/:id/logs`. 실 MySQL E2E: 세션 3건 트랜스크립트(웰컴/user-click/답변) 읽기 렌더 정상, 한글 정상, 콘솔 에러 0. 격리 검증: 소유 봇 200 / **교차 봇 404**(존재 비노출) / 무토큰 401.
...
- [x] ~~대화 로그 조회 UI~~ — 완료(위 2026-07-16 검증 참고). 후속: 검색·필터·기간·페이지네이션·CSV 내보내기(현재 최근 200세션 상한, 읽기 전용).
