...
## 2026-07-13 — ②-b 쓰기 경로 UI E2E 검증 (추가)
- 완성 봇(웰컴 응답 삭제로 미완성 스텝 제거) → UI 저장 → **서버 버전 생성 확인**(토스트 + DB versions 1개), 재로드 시 "버전 1" 로드. 저장→재로드 왕복 OK.
- 발견: 레이아웃 `handlePublish` 가 **어떤 버튼에도 연결돼 있지 않음** → 발행은 UI 트리거 부재. ③에서 버튼 연결.
