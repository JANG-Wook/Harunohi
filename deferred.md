...
## 발견된 갭 (2026-07-13 감사)
- [ ] **위젯용 배포 스냅샷에 대화방 UI(런처) 미포함** ⚠️ — `versionPayload`→`definition_json`→`snapshot_json` 에 scenarios/variables/apis 만 있고 챗룸/버블 스타일(런처)이 없음. 위젯이 **스타일된 챗룸**을 렌더하려면 발행 시 해석된 런처 UI를 스냅샷에 포함해야 함. **공개 챗룸/위젯 청크의 선행 필수.**
