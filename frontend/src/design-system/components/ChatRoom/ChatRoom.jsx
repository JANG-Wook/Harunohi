...
// 인라인 굵기 runs 렌더 — value 가 runs(세그먼트 배열)면 굵기별 span 으로, 평문이면 그대로.
const WEIGHT_VAR = {
...
function renderRich(value) {
  ...
  return value.map((r, i) => (
    <span key={i} style={{ fontWeight: WEIGHT_VAR[r?.weight] ?? WEIGHT_VAR.regular }}>{r?.text ?? ''}</span>
  ))
}
